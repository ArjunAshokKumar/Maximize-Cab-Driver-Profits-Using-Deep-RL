...
class CabDriver():
    ...
    def state_encod_arch2(self, state):
        """convert the state into a vector so that it can be fed to the NN. This method converts a given state-action pair into a vector format. Hint: The vector is of size m + t + d"""
    ...
    def calc_new_time_and_day(self, time_of_day, day_of_week, travel_time):
    ...
    def calc_travel_time(self, current_loc, start_loc, end_loc, time_of_day, day_of_week, Time_matrix):
        """Returns t1: time taken from current location to start locations; t2: time taken from start location to end location"""
        ...
        if current_loc == start_loc:
            ...
            t1 = int(Time_matrix[current_loc-1][start_loc-1][time_of_day][day_of_week])
            time_of_day, day_of_week = self.calc_new_time_and_day(time_of_day, day_of_week, t1)

        t2 = int(Time_matrix[start_loc-1][end_loc-1][time_of_day][day_of_week])
    ...
    def reward_func(self, state, action, Time_matrix):
        ...
        t1, t2 = self.calc_travel_time(current_loc, start_loc, end_loc, time_of_day, day_of_week, Time_matrix)
    ...
    def calc_total_time(self, state, action, Time_matrix):
    ...
    def next_state_func(self, state, action, Time_matrix):
        ...
        if start_loc != 0 or end_loc != 0:
            new_loc = end_loc
        new_time_of_day, new_day_of_week = self.calc_new_time_and_day(time_of_day, day_of_week, total_travel_time)
